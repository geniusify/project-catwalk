...
const YourOutfitList = () => {
  ...
  const addOutfitItem = (id) => {
    ...
    .then((res) => { localStorage.setItem('outfitInfo', setOutfitInfo(outfitInfo => [...outfitInfo, res.data] )); return(res.data) })
    .then((resProduct) => {

      axios({ url: `api/products/` + id + `/styles`, method: 'get' })
      .then((resStyle) => {

        localStorage.setItem('outfitRenderList', setOutfitRenderList(
  ...
  const handleClick = () => {
   ...
   if (productList.length === 0) {
    ...
    localStorage.setItem('productList', setProductList([selectedWindowsItemID]))
    ...
    if (productList.indexOf(selectedWindowsItemID) === -1) {
      ...
      addOutfitItem(selectedWindowsItemID)
      localStorage.setItem('productList', setProductList(productList => [...productList, selectedWindowsItemID]))
    }
   }
  }
 ...
 // }, []);
  ...
  const responsive = {
    ...
    let test = localStorage.getItem('outfitRenderList');
